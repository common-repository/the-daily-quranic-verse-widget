<?php
/*
Plugin Name: The Daily Quranic Verse Widget
Plugin URI: http://iknowledge.islamicnature.com/extras.php
Description: A widget that displays a different Quranic verse a day
Author: Umar Sheikh
Author URI: http://www.indezinez.com
Version: 1.4
*/

add_action('widgets_init', 'load_daily_verse');

function load_daily_verse(){
  if(function_exists('register_widget')){
    register_widget('Daily_Verse');
	}
}

class Daily_Verse extends WP_Widget {

	function Daily_Verse(){
	
		$widget_ops = array('classname' => 'dailyverse', 'description' => __('A widget that displays a different Quranic verse a day', 'dailyverse'));
		$control_ops = array('width' => 300, 'height' => 350, 'id_base' => 'daily-verse');
		$this->WP_Widget('daily-verse', __('Daily Verse', 'dailyverse'), $widget_ops, $control_ops);
	...
	function widget($args, $instance){
      ...
      if(function_exists('file_get_contents')){
        if($file = @file_get_contents('http://iknowledge.islamicnature.com/dv_script.php?lang='.$lang)){
          $file = preg_replace('/document\.write\(\'(.*)\'\)\;/','$1',$file);
          echo '<p>'.str_replace("\'","'",$file).'</p>';
        }else{
          echo '
          <p>
          <script type="text/javascript" src="http://iknowledge.islamicnature.com/dv_script.php?lang='.$lang.'"></script>
	...
	function form($instance){
		...
		$defaults = array('title' => __('Daily Quranic Verse', 'dailyverse'),'lang' => 'english');
		...
		<!-- Language: Select Box -->
		<p>
			<label for="<?php echo $this->get_field_id('lang'); ?>"><?php _e('Language:', 'dailyverse'); ?></label> 
